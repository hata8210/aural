...
export function VoiceInterface({
  ...
  const [drawings, setDrawings] = useState<Drawing[]>(
    () =>
      initialDrawings?.length
        ...
        : [{ id: randomUUID(), label: "Drawing 1", snapshotData: null }],
  );
  ...
  const [codeSnippets, setCodeSnippets] = useState<CodeSnippet[]>([
    { id: randomUUID(), label: "Snippet 1", snapshotData: null },
  ]);
  ...
  const handleAIResponse = useCallback((text: string) => {
    ...
    setMessages((prev) => {
      ...
      if (last?.role !== "assistant") {
        return [...prev, { id: randomUUID(), role: "assistant", content: trimmed, source: "voice" }];
      }
  ...
  const addNewCodeSnippet = useCallback(() => {
    ...
    setCodeSnippets((prev) => {
      ...
      return [...updated, { id: randomUUID(), label: `Snippet ${updated.length + 1}`, snapshotData: null }];
    });
    if (currentSnippet && currentSnapshot) persistCodeSnippet(currentSnippet, currentSnapshot);
    ce.resetScene();
  ...
  const handleSendChat = useCallback((text: string) => {
    ...
    const msg: Message = { id: randomUUID(), role: "user", content: trimmed, source: "chat" };
  ...
  useEffect(() => {
    ...
    if (questionChanged) {
      ...
      if (saved) {
        ...
        const freshDrawings = [{ id: randomUUID(), label: "Drawing 1", snapshotData: null as string | null }];
        const freshSnippets = [{ id: randomUUID(), label: "Snippet 1", snapshotData: null as string | null }];
