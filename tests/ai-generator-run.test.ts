...
test("getGeneratorModelChain respects AI_GENERATOR_MODEL override", () => {
  withEnv(
    {
      OPENAI_API_KEY: "test-openai",
      AI_GENERATOR_MODEL: "qwen-plus",
    },
    () => {
      assert.deepEqual(getGeneratorModelChain(), ["qwen-plus"]);
      assert.equal(resolveGeneratorModel(), "qwen-plus");
    },
  );
});
